...
// function newTDArray (object) {
//   tdArray.push(new NewTD(object.id || tdCount, object.title || $('#title-input').val(), object.task || $('#task-input').val(), object.importance));
// }

function tdTemplate (tdCreator) {
...
function createTD(obj) {
  ...
  return $("<article id=" + id + " class='td-box'>"
      + "<div class='flexer'>"
        + "<h2 class='td-title' contenteditable='true'>" + title + "</h2>"
      + "</div>"
      + "<p class='td-task' contenteditable='true'>" + task + "</p>"
      + "<div class='importance-container'>"
      + "<h4 class='importance-rating'>" + importance + "</h4></div></article>"
    );
}
...
function loadStorage() {
  for (let i = 0; i < localStorage.length; i++) {
    if (localStorage.key(i) !== 'tdCountTracker') {
      ...
      tdTemplate(createTD(storedInfo));
