...
function completeTD(arg) {
  let $completeButton = arg.parent().children('.completed-button');
  let $storedID = arg.parent().parent().attr('id');
  updateTD($storedID, 'completed', 'true');
  $completeButton.prop('disabled', true);
}
...
$('.td-container').on('click', '.completed-button', function() {
  let $completedID = $(this).parent().parent().attr('id');
  completeTD($(this));
});
