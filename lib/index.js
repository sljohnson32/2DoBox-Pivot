...
function voteChange(upVote, downVote) {
  $('.importance-container').on('click', '.up-button, .down-button', function() {
    let parentSelector = $(this).parent().parent();
    let id = parentSelector.attr('id');
    let storedArray = getStorage();
    let currentImportance = findTD(id, storedArray).importance;
    let newImportance;
    if ($(this).is(upVote)) {
