...
//we COULD update this so that completes do NOT show after hitting the button again.  This is secondary to unit testing and css though.
...
//we COULD add a "show all" button to resolve the issue we are having now. this is secondary to unit testing and css.
$('#filter-container').on('click', '#none-filter-button, #low-filter-button, #normal-filter-button, #high-filter-button, #critical-filter-button', function() {
  for (let i=0; i < localStorage.length; i++) {
    ...
    utilities.toggleShowHide(clickedButtonImportance, storedTDImportance, storedTDElement);
  }
});
...
//right now search is pulling up completed TDs as well - even when they are hidden.  need to try to resolve although I think this is secondary to testing and CSS
